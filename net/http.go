...
func SetTransportDefaults(t *http.Transport) *http.Transport {
	...
	if s := os.Getenv("DISABLE_HTTP2"); len(s) > 0 {
		logger.Logf(logger.InfoLevel, "HTTP2 has been explicitly disabled")
	} else if allowsHTTP2(t) {
		if err := configureHTTP2Transport(t); err != nil {
			logger.Logf(logger.WarnLevel, "Transport failed http2 configuration: %v", err)
		}
	}
	return t
}
...
func readIdleTimeoutSeconds() int {
	...
	if s := os.Getenv("HTTP2_READ_IDLE_TIMEOUT_SECONDS"); len(s) > 0 {
		...
		if err != nil {
			logger.Logf(logger.WarnLevel, "Illegal HTTP2_READ_IDLE_TIMEOUT_SECONDS(%q): %v."+
...
func pingTimeoutSeconds() int {
	...
	if s := os.Getenv("HTTP2_PING_TIMEOUT_SECONDS"); len(s) > 0 {
		i, err := strconv.Atoi(s)
		if err != nil {
			logger.Logf(logger.WarnLevel, "Illegal HTTP2_PING_TIMEOUT_SECONDS(%q): %v."+
				" Default value %d is used", s, err, ret)
			return ret
		}
		ret = i
...
func CloseIdleConnectionsFor(transport http.RoundTripper) {
	...
	default:
		logger.Logf(logger.WarnLevel, "unknown transport type: %T", transport)
	}
}
