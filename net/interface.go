...
func isInterfaceUp(intf *net.Interface) bool {
	...
	if intf.Flags&net.FlagUp != 0 {
		logger.Logf(logger.DebugLevel, "Interface %v is up", intf.Name)
		return true
	}
	return false
}
...
func getMatchingGlobalIP(addrs []net.Addr, family AddressFamily) (net.IP, error) {
	if len(addrs) > 0 {
		for i := range addrs {
			logger.Logf(logger.DebugLevel, "Checking addr  %s.", addrs[i].String())
			...
			if memberOf(ip, family) {
				if ip.IsGlobalUnicast() {
					logger.Logf(logger.DebugLevel, "IP found %v", ip)
					return ip, nil
				} else {
					logger.Logf(logger.DebugLevel, "Non-global unicast address found %v", ip)
				}
			} else {
				logger.Logf(logger.DebugLevel, "%v is not an IPv%d address", ip, int(family))
			}
...
func getIPFromInterface(intfName string, forFamily AddressFamily, nw networkInterfacer) (net.IP, error) {
	...
	if isInterfaceUp(intf) {
		...
		logger.Logf(logger.DebugLevel, "Interface %q has %d addresses :%v.", intfName, len(addrs), addrs)
		matchingIP, err := getMatchingGlobalIP(addrs, forFamily)
		if err != nil {
			return nil, err
		}
		if matchingIP != nil {
			logger.Logf(logger.DebugLevel, "Found valid IPv%d address %v for interface %q.", int(forFamily), matchingIP, intfName)
...
func getIPFromLoopbackInterface(forFamily AddressFamily, nw networkInterfacer) (net.IP, error) {
	...
	for _, intf := range intfs {
		...
		if intf.Flags&(net.FlagLoopback) != 0 {
			...
			logger.Logf(logger.DebugLevel, "Interface %q has %d addresses :%v.", intf.Name, len(addrs), addrs)
			matchingIP, err := getMatchingGlobalIP(addrs, forFamily)
			if err != nil {
				return nil, err
			}
			if matchingIP != nil {
				logger.Logf(logger.DebugLevel, "Found valid IPv%d address %v for interface %q.", int(forFamily), matchingIP, intf.Name)
				return matchingIP, nil
			}
		}
	}
...
func chooseIPFromHostInterfaces(nw networkInterfacer, addressFamilies AddressFamilyPreference) (net.IP, error) {
	...
	for _, family := range addressFamilies {
		logger.Logf(logger.DebugLevel, "Looking for system interface with a global IPv%d address", uint(family))
		for _, intf := range intfs {
			if !isInterfaceUp(&intf) {
				logger.Logf(logger.DebugLevel, "Skipping: down interface %q", intf.Name)
				continue
			}
			if isLoopbackOrPointToPoint(&intf) {
				logger.Logf(logger.DebugLevel, "Skipping: LB or P2P interface %q", intf.Name)
				continue
			}
			addrs, err := nw.Addrs(&intf)
			if err != nil {
			...
			if len(addrs) == 0 {
				logger.Logf(logger.DebugLevel, "Skipping: no addresses on interface %q", intf.Name)
				continue
			}
			for _, addr := range addrs {
				...
				if !memberOf(ip, family) {
					logger.Logf(logger.DebugLevel, "Skipping: no address family match for %q on interface %q.", ip, intf.Name)
					continue
				}
				// TODO: Decide if should open up to allow IPv6 LLAs in future.
				if !ip.IsGlobalUnicast() {
					logger.Logf(logger.DebugLevel, "Skipping: non-global address %q on interface %q.", ip, intf.Name)
					continue
				}
				logger.Logf(logger.DebugLevel, "Found global unicast address %q on interface %q.", ip, intf.Name)
				return ip, nil
			}
		}
	}
...
func chooseHostInterfaceFromRoute(routes []Route, nw networkInterfacer, addressFamilies AddressFamilyPreference) (net.IP, error) {
	for _, family := range addressFamilies {
		logger.Logf(logger.DebugLevel, "Looking for default routes with IPv%d addresses", uint(family))
		for _, route := range routes {
			if route.Family != family {
				continue
			}
			logger.Logf(logger.DebugLevel, "Default route transits interface %q", route.Interface)
			finalIP, err := getIPFromInterface(route.Interface, family, nw)
			if err != nil {
				return nil, err
			}
			if finalIP != nil {
				logger.Logf(logger.DebugLevel, "Found active IP %v ", finalIP)
				return finalIP, nil
			}
			...
			if loopbackIP != nil {
				logger.Logf(logger.DebugLevel, "Found active IP %v on Loopback interface", loopbackIP)
				return loopbackIP, nil
			}
		}
	}
	logger.Logf(logger.DebugLevel, "No active IP found by looking at default routes")
