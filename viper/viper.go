...
func StringToBoolHookFunc() mapstructure.DecodeHookFunc {
	return func(
		...
		boolStr := strings.ToLower(data.(string))
		switch boolStr {
		case "yes", "y", "true", "t":
			return true, nil
		case "false", "f", "no", "n":
			return false, nil
		default:
			return false, fmt.Errorf("fail to parse str:%s to bool", boolStr)
		}
...
func (v *Viper) AllSettings() map[string]interface{} {
	...
	confType := strings.ToLower(v.getConfigType())
	var envConf bool
	if confType == "env" {
		envConf = true
	}
	...
	for _, k := range v.AllKeys() {
		...
		if envConf {
			if v.envKeyReplacer != nil {
				lastKey = v.envKeyReplacer.Replace(lastKey)
			}
		}
