...
func (bfr *BoundedFrequencyRunner) Loop(stop <-chan struct{}) {
	// logger.Log(logger.DebugLevel, "%s Loop running", bfr.name)
	bfr.timer.Reset(bfr.maxInterval)
	for {
		select {
		case <-stop:
			bfr.stop()
			// logger.Log(logger.DebugLevel, "%s Loop stopping", bfr.name)
			return
		case <-bfr.timer.C():
			bfr.tryRun()
		case <-bfr.run:
...
func (bfr *BoundedFrequencyRunner) doRetry() {
	...
	if retryInterval < bfr.timer.Remaining() {
		// logger.Log(logger.DebugLevel, "%s: retrying in %v", bfr.name, retryInterval)
		bfr.timer.Stop()
...
func (bfr *BoundedFrequencyRunner) tryRun() {
	...
	if bfr.limiter.TryAccept() {
		...
		bfr.timer.Stop()
		bfr.timer.Reset(bfr.maxInterval)
		// logger.Log(logger.DebugLevel, "%s: ran, next possible in %v, periodic in %v", bfr.name, bfr.minInterval, bfr.maxInterval)
		return
	}
	...
	// logger.Log(logger.DebugLevel, "%s: %v since last run, possible in %v, scheduled in %v", bfr.name, elapsed, nextPossible, nextScheduled)
