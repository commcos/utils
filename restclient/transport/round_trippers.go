...
func (rt *authProxyRoundTripper) CancelRequest(req *http.Request) {
	if canceler, ok := rt.rt.(requestCanceler); ok {
		...
		logger.Log(logger.ErrorLevel, "CancelRequest not implemented by %T", rt.rt)
	}
}
...
func (rt *impersonatingRoundTripper) CancelRequest(req *http.Request) {
	if canceler, ok := rt.delegate.(requestCanceler); ok {
		...
		logger.Log(logger.ErrorLevel, "CancelRequest not implemented by %T", rt.delegate)
	}
}
...
func (rt *debuggingRoundTripper) CancelRequest(req *http.Request) {
	if canceler, ok := rt.delegatedRoundTripper.(requestCanceler); ok {
		...
		logger.Log(logger.ErrorLevel, "CancelRequest not implemented by %T", rt.delegatedRoundTripper)
	}
}
...
func (rt *debuggingRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if rt.levels[debugJustURL] {
		logger.Log(logger.InfoLevel, "%s %s", reqInfo.RequestVerb, reqInfo.RequestURL)
	}
	if rt.levels[debugCurlCommand] {
		logger.Log(logger.InfoLevel, "%s", reqInfo.toCurl())

	}
	if rt.levels[debugRequestHeaders] {
		logger.Log(logger.InfoLevel, "Request Headers:")
		for key, values := range reqInfo.RequestHeaders {
			for _, value := range values {
				logger.Log(logger.InfoLevel, "    %s: %s", key, value)
			}
		}
	}
	...
	if rt.levels[debugURLTiming] {
		logger.Log(logger.InfoLevel, "%s %s %s in %d milliseconds", reqInfo.RequestVerb, reqInfo.RequestURL, reqInfo.ResponseStatus, reqInfo.Duration.Nanoseconds()/int64(time.Millisecond))
	}
	if rt.levels[debugResponseStatus] {
		logger.Log(logger.InfoLevel, "Response Status: %s in %d milliseconds", reqInfo.ResponseStatus, reqInfo.Duration.Nanoseconds()/int64(time.Millisecond))
	}
	if rt.levels[debugResponseHeaders] {
		logger.Log(logger.InfoLevel, "Response Headers:")
